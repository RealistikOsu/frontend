...
var keyPlurals = {
    ...
    singlePageSnippets = {
        ...
        "/rank_request" : function() {
            ...
            var re = /^https?:\/\/(ppy|ussr).(pl|sh)\/(b|beatmaps)\/(\d+)$/gi;
            var reNew = /^https?:\/\/(ppy|ussr).(sh|pl)\/beatmapsets\/(\d+)#(osu|taiko|mania|fruits)\/(\d+)$/gi;
            $("#b-form")
              .submit(function(e) {
                e.preventDefault(); // Fuck osu subdomain
                var value = $("#beatmap").val().trim().replace("osu.", "");
                var reOld = re.exec(value);
                var re2 = reNew.exec(value);
                if (reOld === null && re2 === null) {
                  showMessage(
                    "error",
                    "Please provide a valid link, in the form " +
                            "of either https://osu.(ppy.sh|ussr.pl)/(b|beatmaps)/&lt;ID&gt; or any newer bancho link!");
                ...
                var postData = {};
                if (reOld != null) {
                    postData.id = +reOld.slice(-1).pop()
                } else if (re2 != null) {
                     postData.set_id = +re2.slice(-2).pop()
                }
