...
function updateChartData(mode) {
	...
	.then((out) => {
	    ...
	    window.chart.options.scales.yAxes[0].ticks.max = Math.max(...label) + 10; // Long boi.
	    window.chart.update();
	})
	.catch(err => { throw err }); 
}
...
function loadMostPlayedBeatmaps(mode) {
	...
	api('users/most_played', {id: userID, mode: mode, p: currentPage[mode].mostPlayed, l: 5, rx: 1}, function (resp) {
...
function downloadStar(id) {
	return "<a href='/web/replays_relax/" + id + "' class='new downloadstar'><i class='star icon'></i>" + T("Get") + "</a>";
}
