...
$(document).ready(function() {
	...
	if (wl.search.indexOf("mode=") === -1) {
		if (favouriteMode in range(4, 1)) {
			// Change to std as other modes doesnt exist on ap.
			favouriteMode = 0;
		}
		window.history.replaceState('', document.title, newPathName + "?mode=" + favouriteMode + wl.hash);
	}
	else if (wl.search.split("=")[1] in range(4, 1))
		window.history.replaceState('', document.title, newPathName + "?mode=0" + wl.hash);
...
function range(number, startIdx = 0) {
    return [...Array(number).keys()].map(i => i + startIdx);
}
...
function downloadStar(id) {
	return "<a href='/web/replays_auto/" + id + "' class='new downloadstar'><i class='star icon'></i>" + T("Get") + "</a>";
}
